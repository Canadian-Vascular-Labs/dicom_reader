...
def list_doctors(
    ...
    include_CPSOs: Optional[List[str]] = Query(None, description="Filter by CPSO number"),
):
    print(
...
def list_doctors_by_name_prefix(request, prefix: str):
    return list(
        Doctor.objects.filter(name__icontains=prefix).values_list(
            "name", flat=True
        )
    )
...
def list_locations(request):
    ...
    fsa_locations = [loc[:3].upper() for loc in locations if loc and len(loc) >= 3]
