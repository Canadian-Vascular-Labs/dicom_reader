...
class ImportLog(models.Model):
    # foreign key to doctor (CPSO number)
    doctor = models.ForeignKey(
        Doctor, on_delete=models.CASCADE, related_name="import_logs"
    )
    # whenever a record is touched, set the timestamp to now
    timestamp = models.DateTimeField(auto_now=True)
    # task ID for tracking which request this log belongs to
    task_id = models.CharField(max_length=100, blank=True, null=True)

    def __str__(self):
        return f"{self.doctor.name} ({self.doctor.cpso_number}) imported at {self.timestamp}"
